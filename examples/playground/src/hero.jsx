// Hero with looping animated diagram showing the strategy loop.
      ...
      <svg viewBox="0 0 400 400" aria-hidden="true">
        ...
        {[60, 120, 180].map((r, i) => (
          <circle key={r} cx="200" cy="200" r={r}
            ...
            strokeDasharray="2 6" opacity={0.6 - i * 0.15} />
        ))}
            ...
            <g key={n.id} transform={`translate(${n.x},${n.y})`}>
              {active && (
                <circle r="34" fill="var(--accent-glow)" filter="url(#glow)">
                  <animate attributeName="r" from="22" to="40" dur="1.4s" repeatCount="1" />
                  <animate attributeName="opacity" from="0.6" to="0" dur="1.4s" repeatCount="1" />
                </circle>
              )}
              <circle r="22"
                fill="var(--bg-elev)"
        ...
        <g transform="translate(200,200)">
          <text textAnchor="middle" fontFamily="var(--font-mono)" fontSize="10" fill="var(--fg-dim)" letterSpacing="2" y="-6">FEEDBACK</text>
          <text textAnchor="middle" fontFamily="var(--font-mono)" fontSize="10" fill="var(--fg-dim)" letterSpacing="2" y="8">STRATEGY</text>
          <text textAnchor="middle" fontFamily="var(--font-mono)" fontSize="22" fontWeight="600" fill="var(--accent)" y="34">{phase + 1}/4</text>
        </g>
      </svg>
    </div>
  );
      ...
      <div className="hero-inner">
        <div>
          <div className="hero-eyebrow">
            <span className="pip" />
            <span>v0.4.2 · MIT · TypeScript</span>
          </div>
          <h1>
            Intercept the agent. <em>Reshape the loop.</em>
          </h1>
          <p className="hero-pitch">
            <span className="mono" style={{ color: "var(--fg)" }}>agent-feedback-ui</span> hands the user a
            domain-shaped modal at the moment the agent is about to act — and gives you four interchangeable
            strategies for what happens next.
          </p>
